...
function createVesselLayer(viewer: Cesium.Viewer, root: HTMLElement) {
  ...
  async function renderContacts() {
    ...
    for (const [key, value] of contacts) {
      if (now - Number(value.__seenAt || 0) > 90_000) contacts.delete(key);
    }
    ...
    viewer.dataSources.remove(source,true);
    source = next;
    status = { count, source: 'AISStream · persistent snapshot' };
  ...
  async function pollSnapshot() {
    try {
      ...
      const payload = await r.json().catch(() => ({}));
      if (!r.ok && payload?.unavailable !== false) throw new Error(payload?.error || `HTTP ${r.status}`);

      const rows = Array.isArray(payload?.rows) ? payload.rows : [];
      const present = new Set<string>();
      for (const row of rows) {
        const mmsi = text(row?.mmsi);
        if (mmsi) present.add(mmsi);
        absorb(row);
      }
      for (const key of contacts.keys()) {
        if (!present.has(key)) contacts.delete(key);
      }
      ...
      if (rows.length) {
        ...
        status = unavailable
          ? { count: 0, source: payload?.source || 'AIS snapshot', error: payload?.error || 'AIS snapshot unavailable' }
          : { count: 0, source: payload?.source || 'AISStream · persistent snapshot' };
        updateContactHud(root,null,status);
      }

      if (payload?.needsCollection && !collectorActive) void kickCollector();
    } catch (error) {
      if (contacts.size === 0) {
        status = { count: 0, error: String(error) };
        updateContactHud(root,null,status);
      ...
      pollTimer = window.setTimeout(()=>void pollSnapshot(), 15_000);
    }
  }

  async function kickCollector() {
    if (collectorActive) return;
    collectorActive = true;
    try {
      const r = await fetch('/api/ais-collector', { cache: 'no-store' });
      const payload = await r.json().catch(() => ({}));
      if (!r.ok) {
        if (contacts.size === 0) {
          status = { count: 0, error: payload?.error || `AIS collector HTTP ${r.status}` };
          updateContactHud(root,null,status);
        }
      } else {
        const rows = Array.isArray(payload?.rows) ? payload.rows : [];
        rows.forEach(absorb);
        if (rows.length) scheduleRender();
        void pollSnapshot();
      }
    } catch (error) {
      if (contacts.size === 0) {
      ...
      collectorTimer = window.setTimeout(()=>void kickCollector(), 12_000);
    }
  }
  ...
  window.addEventListener('beforeunload',()=>{
    ...
    if (collectorTimer !== null) window.clearTimeout(collectorTimer);
...
async function start() {
  ...
  Object.assign(window,{__oceanBrain:{version:'v26',viewer,focusBermuda:()=>focusBermuda(viewer,1),getAir:air.getStatus,getSea:sea.getStatus}});
}
