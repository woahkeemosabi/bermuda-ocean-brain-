...
export async function sampleStream({ source = 'AISStream', apiKey, control = false, durationMs = 12000, mmsi, signal, onRow = () => {}, onStatus = () => {}, WebSocketImpl = WebSocket } = {}) {
  ...
  // AISStream rejects world-sized subscriptions; use a bounded Lisbon
  // reference area for the provider control check.
  const bounds = control ? (native ? [59, 10, 60, 11] : [38.3, -9.7, 39.1, -8.6]) : BOUNDS;
